#include "FalcorMathConstants.hlsli"
#include "FalcorMathHelpers.hlsli"
...
struct DiffuseReflectionLambert : IBxDF
{
    ...
    bool sample(const float3 wi, out float3 wo, out float pdf, out float3 weight, out uint lobe, inout SampleGenerator sg)
    {
        ...
        lobe = (uint)LobeType_DiffuseReflection;
...
struct DiffuseTransmissionLambert : IBxDF
{
    ...
    bool sample(const float3 wi, out float3 wo, out float pdf, out float3 weight, out uint lobe, inout SampleGenerator sg)
    {
        wo = sample_cosine_hemisphere_concentric(sampleNext2D(sg), pdf);
        wo.z = -wo.z;
        lobe = (uint)LobeType_DiffuseTransmission;
...
struct SpecularReflectionMicrofacet : IBxDF
{
    ...
    bool hasLobe(uint lobe) { return (activeLobes & (uint)lobe) != 0; }
    ...
    float3 eval(const float3 wi, const float3 wo)
    {
        ...
        if (!hasLobe(LobeType_SpecularReflection)) return 0.f;
    ...
    bool sample(const float3 wi, out float3 wo, out float pdf, out float3 weight, out uint lobe, inout SampleGenerator sg)
    {
        ...
        weight = 0;
        pdf = 0.f;
        lobe = (uint)LobeType_SpecularReflection;
        ...
        if (alpha == 0.f)
        {
            if (!hasLobe(LobeType_DeltaReflection)) return false;

            wo = float3(-wi.x, -wi.y, wi.z);
            pdf = 0.f;
            weight = evalFresnelSchlick(albedo, 1.f, wi.z);
            lobe = (uint)LobeType_DeltaReflection;
            return true;
        }
#endif

        if (!hasLobe(LobeType_SpecularReflection)) return false;
...
struct SpecularReflectionTransmissionMicrofacet : IBxDF
{
    ...
    float3 eval(const float3 wi, const float3 wo)
    {
        ...
        const bool hasReflection = hasLobe(LobeType_SpecularReflection);
        const bool hasTransmission = hasLobe(LobeType_SpecularTransmission);
    ...
    bool sample(const float3 wi, out float3 wo, out float pdf, out float3 weight, out uint lobe, inout SampleGenerator sg)
    {
        ...
        if (alpha == 0.f)
        {
            const bool hasReflection = hasLobe(LobeType_DeltaReflection);
            const bool hasTransmission = hasLobe(LobeType_DeltaTransmission);
            ...
            pdf = 0.f;
            weight = isReflection ? (1.f) : transmissionAlbedo;
            if (!(hasReflection && hasTransmission)) weight *= (isReflection ? F : 1.f - F);
            wo = isReflection ? float3(-wi.x, -wi.y, wi.z) : float3(-wi.x * eta, -wi.y * eta, -cosThetaT);
            lobe = isReflection ? (uint)LobeType_DeltaReflection : (uint)LobeType_DeltaTransmission;
        ...
        lobe = isReflection ? (uint)LobeType_SpecularReflection : (uint)LobeType_SpecularTransmission;
...
struct FalcorBSDF : IBxDF
{
    ...
    */
    void __init(const ShadingData sd, const StandardBSDFData data)
    {
        ...
        float3 transmissionAlbedo = data.transmission;// sd.mtl.isThinSurface() ? data.transmission : sqrt(data.transmission);                                                                                                  //!!! 
        ...
        pDiffuseReflection = (activeLobes & (uint)LobeType_DiffuseReflection) ? diffuseWeight * dielectricBSDF * (1.f - diffTrans) : 0.f;
        pDiffuseTransmission = (activeLobes & (uint)LobeType_DiffuseTransmission) ? diffuseWeight * dielectricBSDF * diffTrans : 0.f;
        pSpecularReflection = (activeLobes & ((uint)LobeType_SpecularReflection | (uint)LobeType_DeltaReflection)) ? specularWeight * (metallicBRDF + dielectricBSDF) : 0.f;
        pSpecularReflectionTransmission = (activeLobes & ((uint)LobeType_SpecularReflection | (uint)LobeType_DeltaReflection | (uint)LobeType_SpecularTransmission | (uint)LobeType_DeltaTransmission)) ? specularBSDF : 0.f;
    ...
    */
    static uint getLobes(const StandardBSDFData data)
    {
        ...
        uint lobes = isDelta ? (uint)LobeType_DeltaReflection : (uint)LobeType_SpecularReflection;
        if (any(data.diffuse > 0.f) && specTrans < 1.f)
        {
            if (diffTrans < 1.f) lobes |= (uint)LobeType_DiffuseReflection;
            if (diffTrans > 0.f) lobes |= (uint)LobeType_DiffuseTransmission;
        }
        if (specTrans > 0.f) lobes |= (isDelta ? (uint)LobeType_DeltaTransmission : (uint)LobeType_SpecularTransmission);
