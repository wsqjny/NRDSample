...
private:
    ...
    void _renderFrameNRDSample(uint32_t frameIndex);
...
private:
    ...
    bool m_IsIntegrateBRDFInit = false;
};
...
bool Sample::Initialize(nri::GraphicsAPI graphicsAPI)
{
    ...
    // SIGMA
    {
        const nrd::MethodDesc methodDescs[] =
        {
            { nrd::Method::SIGMA_SHADOW, (uint16_t)m_ScreenResolution.x, (uint16_t)m_ScreenResolution.y },
        };
        ...
        ImGui::Begin("Settings (F1 - hide)", nullptr, ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoResize);
        {
            ...
            else
            {
                ImGui::PushID("CAMERA");
                {
                    ...
                    ImGui::Checkbox("Enable FalcorPT 5.1", &m_Settings.enableFalcorPT5_1);
...
void Sample::CreatePipelines()
{
    ...
    { // Pipeline::IndirectRays
        const nri::DescriptorRangeDesc descriptorRanges1[] =
        {
            ...
            { 7, 2, nri::DescriptorType::STORAGE_TEXTURE, nri::ShaderStage::ALL }
        };
    ...
    { // Pipeline::Composition
        const nri::DescriptorRangeDesc descriptorRanges1[] =
        {
            { 0, 13, nri::DescriptorType::TEXTURE, nri::ShaderStage::ALL },
            { 13, 1, nri::DescriptorType::STORAGE_TEXTURE, nri::ShaderStage::ALL }
        };
    ...
    { // Pipeline::CompositionNRDSample
        ...
        nri::ComputePipelineDesc pipelineDesc = {};
        pipelineDesc.pipelineLayout = pipelineLayout;
        pipelineDesc.computeShader = utils::LoadShader(m_DeviceDesc->graphicsAPI, "CompositionNRDSample.cs", shaderCodeStorage);
...
void Sample::CreateDescriptorSets()
{
    ...
    { // DescriptorSet::Composition1NRDSample
        NRI_ABORT_ON_FAILURE(NRI.AllocateDescriptorSets(*m_DescriptorPool, *GetPipelineLayout(Pipeline::Composition), 1, &descriptorSet, 1, nri::WHOLE_DEVICE_GROUP, 0));
        m_DescriptorSets.push_back(descriptorSet);
...
{
    if (m_Settings.enableFalcorPT5_1)
    {
        _renderFrameFalcorPT(frameIndex);
    }
    else
    {
        _renderFrameNRDSample(frameIndex);
    }   
...
void Sample::_renderFrameFalcorPT(uint32_t frameIndex)
{
    ...
    NRI.BeginCommandBuffer(commandBuffer, m_DescriptorPool, 0);
    {
        ...
        { // Shadow denoising
            ...
            m_Sigma.SetMethodSettings(nrd::Method::SIGMA_SHADOW, &shadowSettings);
...
void Sample::_renderFrameNRDSample(uint32_t frameIndex)
{
    std::array<nri::TextureTransitionBarrierDesc, 32> optimizedTransitions = {};
    ...
    NRI.BeginCommandBuffer(commandBuffer, m_DescriptorPool, 0);
    {
        ...
        { // Composition
            ...
            NRI.CmdSetPipelineLayout(commandBuffer, *GetPipelineLayout(Pipeline::CompositionNRDSample));
            NRI.CmdSetPipeline(commandBuffer, *Get(Pipeline::CompositionNRDSample));

            const nri::DescriptorSet* descriptorSets[] = { frame.globalConstantBufferDescriptorSet, Get(DescriptorSet::CompositionNRDSample) };
            NRI.CmdSetDescriptorSets(commandBuffer, 0, helper::GetCountOf(descriptorSets), descriptorSets, nullptr);
